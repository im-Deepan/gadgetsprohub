...
export class FieldExtractors {
  ...
  public static getSiteStripeLink(expectedTag?: string): string | null {
    ...
    if (expectedTag) {
      if (rawLink.includes('amzn.to') || rawLink.includes('link.amazon') || rawLink.includes('a.co')) {
        // SiteStripe shortlinks generated by user on their account already contain their affiliate code
        return rawLink;
      }
