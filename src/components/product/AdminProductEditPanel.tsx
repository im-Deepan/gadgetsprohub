    ...
    <div className="bg-violet-50/50 dark:bg-violet-950/15 border border-violet-50 dark:border-violet-800/40 p-4 rounded-xl -mx-2 sm:-mx-0 space-y-3 mb-6 transition-all font-sans">
      <div className="flex items-center justify-between pointer-events-none">
        ...
        <button
          ...
          className="text-xs font-bold text-violet-600 hover:text-violet-950 dark:text-violet-300 dark:hover:text-violet-200 underline cursor-pointer pointer-events-auto"
        >
      ...
      {isAdminEditVisible && (
        <form onSubmit={handleAdminEditSubmit} className="space-y-2.5 pt-1 text-slate-700 dark:text-slate-50">
          <div>
            <label className="block text-[10px] font-bold uppercase text-slate-300 mb-1">Product Title</label>
            <input
              type="text"
          ...
          <div className="grid grid-cols-3 gap-2">
            ...
            <div>
              <label className="block text-[10px] font-bold uppercase text-slate-300 mb-1">Discount (%)</label>
              <input
                type="number"
                step="0.01"
                min="0"
          ...
          <div>
            <label className="block text-[10px] font-bold uppercase text-slate-300 mb-1">Affiliate Destination Link / Shop URL</label>
            <input
              type="text"
              className="w-full text-xs p-2 rounded-lg border border-indigo-100 dark:border-slate-700 bg-white dark:bg-slate-800 font-mono text-indigo-500 dark:text-indigo-300"
          ...
          <div>
            <label className="block text-[10px] font-bold uppercase text-slate-300 mb-1">Demo Video URL (Direct MP4 clip link)</label>
            <input
              type="text"
              className="w-full text-xs p-2 rounded-lg border border-slate-100 dark:border-slate-700 bg-white dark:bg-slate-800 font-mono"
          ...
          <div>
            <label className="block text-[10px] font-bold uppercase text-slate-300 mb-1">Short Outline Summary</label>
            <input
              type="text"
              className="w-full text-xs p-2 rounded-lg border border-slate-100 dark:border-slate-700 bg-white dark:bg-slate-800"
          ...
          <div>
            <label className="block text-[10px] font-bold uppercase text-slate-300 mb-1">Product Specifications (Semicolon separated parameter=value pairs)</label>
            <textarea
              rows={2}
              placeholder="e.g. Battery=5000 mAh; Weight=185g; OS=Android 14"
              className="w-full text-xs p-2 rounded-lg border border-slate-100 dark:border-slate-700 bg-white dark:bg-slate-800 font-mono text-indigo-500 dark:text-indigo-300"
          ...
          <div>
            <label className="block text-[10px] font-bold uppercase text-slate-300 mb-1">Highlighted Advantages (Comma separated)</label>
            <input
              type="text"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            <div>
              <label className="block text-[10px] font-bold uppercase text-slate-300 mb-1">Verified Pros (Comma separated)</label>
              <textarea
                rows={2}
            ...
            <div>
              <label className="block text-[10px] font-bold uppercase text-slate-300 mb-1">Verified Cons (Comma separated)</label>
              <textarea
                rows={2}
          ...
          {adminEditSuccess && (
            <div className="bg-emerald-50 text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-300 p-2.5 rounded-lg text-center text-[11px] font-bold border border-emerald-100/45">
