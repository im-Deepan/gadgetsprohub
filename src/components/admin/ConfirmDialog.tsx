...
export const ConfirmDialog: React.FC<ConfirmDialogProps> = ({ 
  ...
  return (
    <div className="fixed inset-0 z-[999] flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-xs">
      <div className="w-full max-w-md overflow-hidden rounded-2xl border border-slate-50 bg-white p-6 shadow-xl dark:border-slate-700 dark:bg-zinc-800 transition-all duration-300">
        ...
        <div className="mt-5 flex justify-end gap-3">
          <button
            type="button"
            onClick={onCancel}
            className="rounded-lg bg-slate-50 hover:bg-slate-100 text-slate-600 dark:bg-slate-700 dark:hover:bg-slate-600 dark:text-slate-200 py-2 px-3.5 text-xs font-bold transition-all duration-300 cursor-pointer"
          >
            {cancelText || 'Cancel'}
          </button>
          <button
