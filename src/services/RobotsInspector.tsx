...
export const RobotsInspector: React.FC<{ initialHost?: string }> = ({ initialHost = 'gadgetsprohub.onrender.com' }) => {
  ...
  useEffect(() => {
    const fetchConfig = async () => {
      try {
        ...
        const res = await fetch('/api/admin/seo/robots/config', {
          headers: token ? { Authorization: `Bearer ${token}` } : {}
        });
        if (res.ok) {
          const data = await res.json();
          if (data?.config && data.config.enableAiBots !== undefined) {
            setEnableAi(data.config.enableAiBots);
          }
        }
      } catch (err) {
        // Fallback to default
  ...
  const handleSaveToLive = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await fetch('/api/admin/seo/robots/sync', {
        method: 'POST',
        headers: {
      ...
      console.warn('Failed to sync live robots.txt:', err);
    } finally {
      setIsSaving(false);
    }
  };
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-6 border-b border-slate-200 dark:border-slate-800">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            disabled={isSaving}
            className="flex items-center gap-1.5 px-3.5 py-2 text-xs font-semibold rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white shadow-sm transition-colors disabled:opacity-50"
          >
            {saveSuccess ? (
            ...
            {saveSuccess ? 'Saved & Deployed!' : isSaving ? 'Saving...' : 'Deploy to Live Site'}
          </button>
        </div>
      </div>
...
export default RobotsInspector;
